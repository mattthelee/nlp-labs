...
class CorefModel(object):
  ...
  def get_predictions_and_loss(self,word_embeddings,sent_lengths,mention_starts,mention_ends,mention_cluster_ids,is_training):
    ...
    for i in range(self.ffnn_layer):
      hidden_weights = tf.get_variable("hidden_weights_{}".format(i),[self.shape(ffnn_input,1), self.hidden_size])
    ...
    output_weights = tf.get_variable("output_weights", [self.shape(ffnn_input, 1), 1])
